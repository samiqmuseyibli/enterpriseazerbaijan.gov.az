<?php
    ...
    public function register_user()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect($l.'/user/register');
        } else {

            if (!preg_match('/^(?=.*\d)(?=.*[\W_])(?=.*[a-z])(?=.*[A-Z]).{12,30}$/', $this->input->post('pass'))) {
            $this->session->set_flashdata('error_message', translate('note_for_password'));
             redirect($l.'/user/register');
                }
           ...
           redirect($l.'/user/register');
            ...
            if ($email_check) {
               ...
               redirect($l.'/user/register');
                ...
                $message = "Aktivasiya Linki: <a href=" . base_url() .$l. "/user/activateaccount?hash=" . $user['accept_hash'] . ">" . base_url() .$l. "/user/activateaccount?hash=" . $user['accept_hash'] . "</a>";
                $to = $user['user_mail'];
    ...
    public function login_user()
    {
		...
		if ($this->input->post('redirect_url')) {
			$this->form_validation->set_rules('redirect_url', 'redirectUrl', 'exact_length[15]|in_list[az/user/categories|en/user/categories|ru/user/categories]');
		}
		//#########
        ...
        if($this->form_validation->run() == FALSE) {
			...
			if (!$this->captcha_model->check(html_escape($this->input->post('captchaCode')))) {
				...
				if ($email_check) {
					...
					$message = "Bərpa Linki: <a href= " . base_url() .$l. "/user/passwordreset?resetkey=" . $password_hash . ">" . base_url() .$l."/user/passwordreset?resetkey=" . $password_hash . "</a>";
					...
					if ($mail) {
						...
						$this->session->set_flashdata('error_message', translate('error'));
						redirect($l.'/user/password_reset');
					}
				} else {
    ...
    public function updatepassword()
    {
        ...
        if (!preg_match('/^(?=.*\d)(?=.*[\W_])(?=.*[a-z])(?=.*[A-Z]).{12,30}$/', $this->input->post('pass1'))) {
            $this->session->set_flashdata('error_message', translate('note_for_password'));
              if (isset($parsed_url['host']) && $parsed_url['host'] === $allowed_host) {
                                redirect($referrer);
                            } else {
    ...
    public function user_update_password()
    {
        ...
        if (!preg_match('/^(?=.*\d)(?=.*[\W_])(?=.*[a-z])(?=.*[A-Z]).{12,30}$/', $this->input->post('newpass1'))) {
            $this->session->set_flashdata('error_message', translate('note_for_password'));
             if (isset($parsed_url['host']) && $parsed_url['host'] === $allowed_host) {
                                redirect($referrer);
                            } else {
        ...
        if ($newpass1 === $newpass2) {
            if ($data['user_pass'] === $old_pass) {
                ...
                redirect($l.'/user/password_update');
            }
        } else {
            $this->session->set_flashdata('error_message', translate('invalid_password_data'));
            redirect($l.'/user/password_update');
              ...
              if (!empty($_FILES['images']['tmp_name'][0])) {
                foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
                    if (!is_uploaded_file($tmp_name)) {
                        echo $this->session->set_flashdata('error_message', translate('choose_correct_file_type'));
                        redirect($l.'/user/addidea');
                        exit();
                    }
                }

                if ($this->project_model->upload_files('images', 'projectimages', $project_id) !== true) {
                    echo $this->session->set_flashdata('error_message', translate('choose_correct_file_type'));
                    redirect($l.'/user/addidea');
                    exit();
                }
    ...
    public function addstartap($parametr='')
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_FILES['images']['name'])) {
                ...
                if($say>=6){
                    echo $this->session->set_flashdata('error_message', translate('you_can_download_maximum_5_picture'));
                    redirect($l.'/user/addstartap');exit();
                }
            }
            $project = array(
                'cf_status' => $cf_status,
    ...
    public function addinvestment($parametr='')
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_FILES['images']['name'])) {
                ...
                if($say>=6){
                    echo $this->session->set_flashdata('error_message', translate('you_can_download_maximum_5_picture'));
                    redirect($l.'/user/addinvestment');exit();
                }
            }
            $project = array(
                'cf_status' => $cf_status,
...
////////////////////////////////////////////////////Update_Projects/////////////////////////////////////////////////////  
    ...
    public function update_idea()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
                ...
                redirect(base_url().$l.'/user/editproject/'.$project_id.'/'.$kat['kat_id']);
        }else{
