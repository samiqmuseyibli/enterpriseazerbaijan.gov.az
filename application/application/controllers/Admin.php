...
class Admin extends CI_Controller
{
    ...
    public function updating_password()
    {
        ...
        if (!preg_match('/^(?=.*\d)(?=.*[\W_])(?=.*[a-z])(?=.*[A-Z]).{12,30}$/', $this->input->post('password_1'))) {
             $this->session->set_flashdata('update_datatable', '<div class="alert alert-danger text-center alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                <h4><i class="icon fa fa-check"></i> Xəta!</h4>
               Şifrə minimum 12 simvoldan ibarət olmalıdır və tərkibində ən azı bir kiçik hərf, bir böyük hərf, bir rəqəm və bir xüsusi simvol olmalıdır. </div>');
            redirect('admin/update_password');
                }
    ...
    public function doupload()
    {
        ...
        if (!empty(findJavaScriptCodes($_FILES['file']['tmp_name']))) {
                $this->session->set_flashdata('update_datatable', '<div class="alert alert-danger text-center alert-dismissible">
                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                        <h4><i class="icon fa fa-ban"></i> Xəta!</h4>
                        Bu PDF fayl təhlükəli məzmun ehtiva edir və qəbul edilmir!</div>');
            redirect('admin/uploadedfiles');
            } 
    ...
    public function video_add()
    {
        ...
        $this->load->view('back/video/add/home', $page_data);
    }
    ...
    public function video_adding()
    {
        ...
        $data = array(
            ...
            'v_category'  => html_escape($this->input->post('category')),
    ...
    public function update_video($id = '')
    {
        ...
        $data['categories'] = $this->db->where('vc_status','1')->get('video_category')->result_array();
