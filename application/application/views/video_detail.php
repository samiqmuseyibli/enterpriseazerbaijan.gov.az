...
<div class="page_title2 sty2">
	<div class="container">
		...
		<div class="pagenation">
			<a href="<?=base_url($l);?>"><?=translate('home_page');?></a> 
			<i>/</i> 
			<a href="<?=base_url($l);?>/video"><?=translate('videos');?></a>
		</div>
	</div>
</div>
<div class="clearfix"></div>
			...
			<div class="sidebar_widget">  
				...
				<ul class="recent_posts_list">
					<?php foreach ($rows as $item) { ?>
							<li>
								<span>
									<a href="<?=base_url($l.'/video/detail/'.$item['v_id'])?>">
										<div class="video-thumbnail">
										   <img class="imgframe2" style="max-width:90px" src="<?=base_url($item['v_cover'])?>" alt="<?=$item['v_title_'.$l]?>" />
									    </div>
									</a>
								</span>
								<a href="<?=base_url($l.'/video/detail/'.$item['v_id'])?>"><?=qisalt($item['v_title_'.$l.''], 80) ?></a>
